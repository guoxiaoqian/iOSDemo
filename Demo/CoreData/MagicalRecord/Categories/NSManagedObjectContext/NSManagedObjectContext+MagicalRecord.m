...
+ (NSManagedObjectContext *) MR_defaultContextQQ
{
	@synchronized (self)
	{
    ...
    else
    {
        ...
        iCloudSetupNotificationObserver = [[NSNotificationCenter defaultCenter] addObserverForName:kMagicalRecordPSCDidCompleteiCloudSetupNotification
                                                                       ...
                                                                       usingBlock:^(NSNotification *note) {
                                                                           [[NSManagedObjectContext MR_defaultContextQQ] MR_observeiCloudChangesInCoordinator:coordinator];
                                                                       }];        
    }
...
+ (void)rootContextChanged:(NSNotification *)notification {
    ...
    [[self MR_defaultContextQQ] mergeChangesFromContextDidSaveNotification:notification];
}
...
+ (void) MR_resetDefaultContext
{
    void (^resetBlock)(void) = ^{
        [[NSManagedObjectContext MR_defaultContextQQ] reset];
    };
...
{
    NSManagedObjectContext *context = [[self alloc] initWithConcurrencyType:NSPrivateQueueConcurrencyType];
    [context setParentContext:[self MR_defaultContextQQ]];
    return context;
}
