...
- (BOOL)setString:(NSString *)string forKey:(NSString *)key genericAttribute:(id)genericAttribute label:(NSString *)label comment:(NSString *)comment error:(NSError *__autoreleasing *)error
{
    ...
    NSLog(@"UICKeyChainStore setString:%@ forKey:%@ genericAttribute: label:%@ comment:%@ error:%@", string, key, label, comment, e);
    return NO;
}
...
- (BOOL)setData:(NSData *)data forKey:(NSString *)key genericAttribute:(id)genericAttribute label:(NSString *)label comment:(NSString *)comment error:(NSError *__autoreleasing *)error
{
    if (!key) {
        ...
        NSLog(@"UICKeyChainStore setData:%@ forKey:%@ genericAttribute: label:%@ comment:%@ error1:%@", data, key, label, comment, e);
        return NO;
    }
    if (!data) {
        return [self removeItemForKey:key error:error];
    ...
    if (status == errSecSuccess || status == errSecInteractionNotAllowed) {
        ...
        if (unexpectedError) {
            ...
            NSLog(@"UICKeyChainStore setData:%@ forKey:%@ genericAttribute: label:%@ comment:%@ error2:%@", data, key, label, comment, unexpectedError);
            return NO;
        } else {
            ...
            if (status != errSecSuccess) {
                ...
                NSLog(@"UICKeyChainStore setData:%@ forKey:%@ genericAttribute: label:%@ comment:%@ error3:%@", data, key, label, comment, e);
                return NO;
            }
        }
    } else if (status == errSecItemNotFound) {
        ...
        if (unexpectedError) {
            ...
            NSLog(@"UICKeyChainStore setData:%@ forKey:%@ genericAttribute: label:%@ comment:%@ error4:%@", data, key, label, comment, unexpectedError);
            return NO;
        } else {
            ...
            if (status != errSecSuccess) {
                ...
                NSLog(@"UICKeyChainStore setData:%@ forKey:%@ genericAttribute: label:%@ comment:%@ error5:%@", data, key, label, comment, e);
                return NO;
            }
        }
    } else {
        ...
        NSLog(@"UICKeyChainStore setData:%@ forKey:%@ genericAttribute: label:%@ comment:%@ error6:%@", data, key, label, comment, e);
        return NO;
    }
